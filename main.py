...
# =============================================================================
# Spread config - driven by SPREADS_CONFIG env var (JSON).
#
# Set via: flyctl secrets set SPREADS_CONFIG='[{"pair1":"AUD_JPY","pair2":"NZD_JPY","hedge_ratio":1.05,"entry_z":2.3,"exit_z":0.2}]'
# The spread manager UI (artifact) generates this command for you automatically.
#
# Falls back to the default below if the env var is not set.
# =============================================================================
...
_raw = os.environ.get('SPREADS_CONFIG', '')
if _raw.strip():
    try:
        _SPREADS_RAW = json.loads(_raw)
    except json.JSONDecodeError as e:
        print(f"[ERROR] Could not parse SPREADS_CONFIG env var: {e}")
        print(f"[ERROR] Falling back to default spread config")
        _SPREADS_RAW = _DEFAULT_SPREADS_CONFIG
else:
    _SPREADS_RAW = _DEFAULT_SPREADS_CONFIG


def _derive_instruments(spreads_raw: list) -> list:
    """
    Derive the full instrument list from spread definitions.

    Cross pairs (e.g. AUD_JPY) don't have a direct USD price, so we also
    need to fetch the base_USD leg (e.g. AUD_USD) so that get_usd_value_per_unit()
    can compute dollar-neutral sizing correctly. Without it, beta-weighting
    silently falls back to 1.0 and your leg sizes will be off.
    """
    needed = set()
    for s in spreads_raw:
        for pair in (s['pair1'], s['pair2']):
            needed.add(pair)
            base  = pair[:3]
            quote = pair[4:]
            if quote != 'USD' and base != 'USD':
                needed.add(f"{base}_USD")
    return sorted(needed)


INSTRUMENTS  = _derive_instruments(_SPREADS_RAW)
SPREADS      = [(s['pair1'], s['pair2']) for s in _SPREADS_RAW]
HEDGE_RATIOS = {f"{s['pair1']}/{s['pair2']}": s['hedge_ratio'] for s in _SPREADS_RAW}

# Per-spread z-score overrides from cointegration recommendations.
# Falls back to the global ENTRY_Z_SCORE / EXIT_Z_SCORE if not specified.
SPREAD_ENTRY_Z = {f"{s['pair1']}/{s['pair2']}": s.get('entry_z', ENTRY_Z_SCORE) for s in _SPREADS_RAW}
SPREAD_EXIT_Z  = {f"{s['pair1']}/{s['pair2']}": s.get('exit_z',  EXIT_Z_SCORE)  for s in _SPREADS_RAW}
...
class TradingBot:
    ...
    def __init__(self):
        ...
        # Set up spread analyzers with per-spread z-score params
        for pair1, pair2 in SPREADS:
            spread_name = f"{pair1}/{pair2}"
            self.analyzer.add_spread(
                pair1, pair2,
                lookback=LOOKBACK_PERIODS,
                entry_z=SPREAD_ENTRY_Z.get(spread_name, ENTRY_Z_SCORE),
                exit_z=SPREAD_EXIT_Z.get(spread_name, EXIT_Z_SCORE)
            )

        print("[INIT] Bot initialized")
        print(f"[INIT] Tracking spreads: {[f'{p1}/{p2}' for p1, p2 in SPREADS]}")
        print(f"[INIT] Instruments to fetch: {INSTRUMENTS}")
        ...
        # Print per-spread z-score overrides if any differ from globals
        for spread_name in SPREAD_ENTRY_Z:
            ez = SPREAD_ENTRY_Z[spread_name]
            xz = SPREAD_EXIT_Z[spread_name]
            if ez != ENTRY_Z_SCORE or xz != EXIT_Z_SCORE:
                print(f"[INIT] {spread_name}: entry_z={ez}, exit_z={xz} (overrides)")
        ...
        for pair1, pair2 in SPREADS:
            ...
            if pair1 in position_map and pair2 in position_map:
                ...
                self.open_positions[spread_name] = {
                    ...
                    'entry_z': None,   # Unknown - lost on restart
                    'entry_time': None,
                    'reconciled': True
                }
    ...
    def warm_up(self) -> bool:
        ...
        # INSTRUMENTS includes both spread legs AND any base_USD helpers
        # needed for cross-pair beta weighting (e.g. AUD_USD for AUD_JPY).
        # warm_up only loads spread legs into the analyzers below - the
        # USD helper candles are only needed for live price lookups.
        candle_data = {}
        ...
        # Load into analyzers (only spread legs matter here)
    ...
    def get_usd_value_per_unit(self, instrument: str, prices: dict) -> float:
        ...
        if quote == 'USD':
            ...
            # XXX/YYY (e.g., AUD/JPY) - need XXX/USD rate
            # _derive_instruments() ensures this was fetched
    ...
    def execute_spread_trade(self, signal: SpreadSignal, prices: dict) -> bool:
        ...
        For NEGATIVE correlation (inverse relationship like EUR_USD/USD_CHF):
            ...
            SHORT_SPREAD: Sell pair1, Sell pair2
        ...
        if signal.signal == 'LONG_SPREAD':
            ...
            if is_inverse:
                pair2_units = pair2_adjusted    # Both buy
            else:
                pair2_units = -pair2_adjusted   # Buy pair1, sell pair2
        elif signal.signal == 'SHORT_SPREAD':
            pair1_units = -TRADE_UNITS
            if is_inverse:
                pair2_units = -pair2_adjusted   # Both sell
            else:
                pair2_units = pair2_adjusted    # Sell pair1, buy pair2
        ...
        if signal.signal != 'CLOSE':
            inverse_note = " (INVERSE pair)" if is_inverse else ""
            print(f"[TRADE {now}] Beta weighting: {signal.pair1}=${pair1_usd_value:.4f}/unit, "
                  f"{signal.pair2}=${pair2_usd_value:.4f}/unit{inverse_note}")
